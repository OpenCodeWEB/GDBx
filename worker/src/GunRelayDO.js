 ...
 * directly (`wss://…/gun`) without running any separate relay — and without
 * Workers KV, D1 or any other database. Storage is Durable Object SQLite,
 * exactly like every other GDBx engine on this worker.
 *
 * Implements the GunDB wire protocol directly (no Node runtime required):
 *   - state-based LWW graph merge (gun `_._[">"]` semantics)
 *   - read/`get` responses, put acks, broadcast/relay between connected peers
 *   - parent lexicon materialization (so `.map()` sees children on fresh clients)
 *   - persistent graph storage in Durable Object SQLite (hibernation WebSocket API)
 *
 * Originally adapted from ankushKun/gun
 * (https://github.com/ankushKun/gun, MIT License) via GunX
 * (github.com/OpenCodeWEB/GunX, MIT) — absorbed into GDBx 2026-08-22.
 *
  ...
  async fetch(request) {
    ...
    if ((url.pathname === "/api/stats" || url.pathname === "/gun/stats") && request.method === "GET") {
      return json({
        ...
        graphNodes: this.stats.graphNodes,
        // No Workers KV / D1 anywhere — one sovereign DO SQLite substrate.
