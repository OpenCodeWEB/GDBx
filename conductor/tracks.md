...
| org-gdbx-unification | Phase 6: org-wide GDBx fabric â€” AiA & OS bridges, Python SDK, all OpenCodeWEB projects | completed | [spec](./tracks/org-gdbx-unification/spec.md) | [plan](./tracks/org-gdbx-unification/plan.md) |
...
| gdbx-playground | Phase 6.2: Live P2P Playground â€” GunX parity (chat, rooms, presence, file) | completed | [spec](./tracks/gdbx-playground/spec.md) | [plan](./tracks/gdbx-playground/plan.md) |
| gdbx-ftp | Phase 6.3: Sovereign FTP Gateway â€” ftp-srv + GDBx + chunked pool | completed | [spec](./tracks/gdbx-ftp/spec.md) | [plan](./tracks/gdbx-ftp/plan.md) |
| gun-compat-engine | Phase 6.4: GunX serverless relay absorbed â€” /gun wire protocol, gun.js clients peer directly | completed | â€” (see commit 6af063b) | â€” |
| gdbx-interconnect | Phase 6.5: Unified Interconnect â€” Global Secure Live Sync (any host + local) | plan_ready | [spec](./tracks/gdbx-interconnect/spec.md) | [plan](./tracks/gdbx-interconnect/plan.md) |

## Live deployment notes (2026-08-21)

- **Pages â†’ DO 500 root cause**: git-push deploys do NOT inject `wrangler.toml` DO bindings
...
- **WS live sync**: WebSocket hub moved INSIDE `GDBxStorageObject` (singleton isolate) â€”
...
- **One firewall, all transports**: `FirewallGuard` (PoWâ†’replayâ†’sigâ†’RBACâ†’ACL) gates HTTP `/sync`,
  ...
  (superadmin-signed). ROOT_PUBKEYS not yet set in production â€” promote/demote live later.
...
- **Hybrid mesh (live)**: Nostr kind-23124 events carry GDBx envelopes; `/relay` verified
  live (signed event â†’ applied delta). WebRTC signaling builders ready in `sdk/transport.js`.
- **Verified**: LIVE E2E PASS (registerâ†’putâ†’getâ†’statsâ†’purgeâ†’resolve), 84/84 unit tests,
...
- Commits: `3ea71de` â†’ `0d1a6a6` (Phase 4), `d299500` â†’ `2e40c8b` (Phase 5 local, pending push)