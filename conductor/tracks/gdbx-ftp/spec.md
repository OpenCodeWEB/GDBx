...
> **Top 1:** Node.js Local Bridge Adapter (ftp-srv + WebCrypto) — 100% FileZilla compatible, bypasses Workers TCP limit, keeps GDBX1 zero-trust

## 1. Context & Why FTP

FTP is legacy but still used by FileZilla, enterprise pipelines, IoT. GDBx already routes WS+Nostr+WebRTC — adding FTP as hybrid-mesh transport makes `.GDBx` truly "One address. Every technology." Gemini proposed 5 alternatives; Top 1 is local loopback gateway (127.0.0.1:2121) that FileZilla connects to, bridge converts FTP commands to GDBX1-signed deltas.
...
- FileZilla: Host `127.0.0.1:2121`, Username `<addr>.gdbx`, Password `<priv>` or session token → local bridge captures, GDBX1-signs, encrypts, puts to GDBx mesh
...
- File binary **not** in LWW delta directly. Delta stores **Manifest** at `sys/ftp/manifest/<filepath_hash>`:
  ...
  ```
- Chunks: AES-GCM encrypted (key from room? or per-file random IV + GDBX1-signed manifest), BLAKE3 hashed, stored in `GDBxMirrorDO` pool or Cloudflare R2 (or P2P WebRTC if R2 not configured) — Chunk size 256KB (adaptive like GunX's file)
- `GDBX1 Signed FTP Commands`: `{op: "FTP_STOR", path, manifest}` signed via `GDBxCrypto.sign` (like playground's PoW+sig)
...
## 4. Constraints
- **No plain-text over network**: FTP plain-text only on 127.0.0.1 loopback, immediately GDBX1-signed + AES-GCM encrypted
...
1. **Node.js Local Bridge Adapter (ftp-srv + WebCrypto)** — PRO: 100% FileZilla compatible, bypasses Workers TCP limit, keeps GDBX1 zero-trust — **Top 1** ✅
...
- [ ] `sdk/ftp_bridge.js` — `GDBxFTP` put/get/sync, GDBX1 signed, 5+ tests (mock)
